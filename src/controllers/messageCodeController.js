...
class MessageCodeController {
  ...
  async sendMessageCode(ctx) {
    ...
    await messageCodeService.sendCode(phone, code)
      .then(res => callback(res), err => error(err))
      .catch(err => {
        console.log("发送验证码出错");
        console.log(err);
      })
   ...
   * 校验验证码
   */
  async checkMessageCode(ctx) {
    const { phone, messageCode: inputCode } = ctx.request.body;
    ...
    } else if (destCode !== inputCode) {// 比较短信验证码
      ctx.response.body = {
        code: -1,
        message: "短信验证码校验失败",
        success: false
